...
def robotPDFeedbackControl(xi, positions, n, a, GOAL_DIST_THRESHOLD=0.08, K_RO=2, K_ALPHA=4, Kd_RO = 1.5, Kd_ALPHA = 8, V_CONST=0.25):
    _,N = np.shape(xi)
    dxi = np.zeros((2, N))
...
def set_velocities(ids, velocities, max_linear_velocity = 0.3, max_angular_velocity = 1.5):
...
if __name__ == '__main__':
    try:
        ...
        while not rospy.is_shutdown():
            ...
            put_velocities(N, k)
            print (k)
            if(np.linalg.norm(pose[0:2, 0:N]-x_goal)< 0.1):
                sleep(1)
